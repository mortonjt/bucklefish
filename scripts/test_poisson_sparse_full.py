...
from poisson_sparse_full import PoissonRegression
...
class PoissonRegressionTest(tf.test.TestCase):
    ...
    def test_sample(self):
        ...
        for _ in range(10):
            with tf.Graph().as_default(), tf.Session() as sess:
                ...
                model = PoissonRegression(sess, batch_size=4, num_neg_samples=3)
                model.N = N
                model.M = N
                model.D = D

                batch = model.sample(y_data)
                pos_res = sess.run(batch)

                # convert to tables with 3 columns (row, col, data)
                res_pos = np.hstack((pos_res.indices, pos_res.values.reshape(-1, 1)))
                pos_row = pos_res.indices[:, 0]
                pos_col = pos_res.indices[:, 1]
                pos_data = pos_res.values

                exp = set((x, y) for (x, y) in zip(row, col))
                res = set(tuple(x) for x in pos_res.indices)

                self.assertTrue(set(res).issubset(set(exp)))
                self.assertTrue(set(pos_data).issubset(set(data)))
    ...
    def test_inference(self):
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            model = PoissonRegression(
                sess, batch_size=5, num_neg_samples=3,
            ...
            # Really simple tests to make sure that something pops out
            # There is no optimization done, so we just want to make sure
            # that the outputs are nonzero.
    ...
    def test_optimize(self):
        ...
        p = md.shape[1]
        table = coo_matrix(table)
        # set random seed, since stochastic gradient descent can
        # be finicky.
        np.random.seed(0)
        for _ in range(10):
            with tf.Graph().as_default(), tf.Session() as sess:
                ...
                model = PoissonRegression(
                    sess,
                    batch_size=5, num_neg_samples=3,
                    learning_rate=1e-1,
                    clipping_size=10,
                    beta_mean=0, beta_scale=1,
                    gamma_mean=0, gamma_scale=1
    ...
    def test_with_simulation(self):
        ...
        ex = random_poisson_model(
            ...
            reps=1, low=-1, high=1,
            alpha_mean=-4, alpha_scale=0,
            theta_mean=0, theta_scale=2,
            gamma_mean=0, gamma_scale=1,
        ...
        with tf.Graph().as_default(), tf.Session() as sess:
            ...
            model = PoissonRegression(
                ...
                clipping_size=10,
                save_path='tf_debug',
                beta_mean=0, beta_scale=10,
                gamma_mean=0, gamma_scale=10)
            model.N = N
            model.M = N
            model.D = D
            model.p = p
            ...
            log_loss = model.loss(G_data, y_data,
                                  positive_batch, random_batch)
            train, g, v = model.optimize(log_loss)
            tf.global_variables_initializer().run()
            train_, grads, loss_1, beta, gamma = sess.run(
                [train, g, log_loss, model.qbeta, model.qgamma]
            ...
            # Notes : Pearson correlation won't give a perfect indication
            # At the moment, this run is facing issues with potential biases
            # in intercept calculations.  There are also issues with
            # coefficient calculations with large ranges of values
            # (i.e. when the range of betas is high).
